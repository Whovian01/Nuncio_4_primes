 ...
 */
package nuncio_4_method_madness;
...
public class Nuncio_4_Method_Madness extends Application {
    ...
    private void mathyCircle(GraphicsContext gc, double width, double height) {
        double x0 = 0.5 * (width - 5);
        double y0 = 0.5 * (height - 10);
        for (int sx = 0; sx < width; sx++) {
            for (int sy = 0; sy < height; sy++) {
                ...
                if ((tx + ty) % 3 == 0) {
                    drawSomeGreenCircle(gc, tx, ty, 5);
                   // System.out.println(tx + ty);
                }  else  if ((tx + ty) % 2 == 0) {
                    drawSomeRedCircle(gc, tx, ty, 50);
                    //System.out.println(tx + ty);
                } 
                else {
                    drawSomeBlueCircle(gc, tx, ty, 50);
                }
            }
        }
    }
    ...
    private void drawShapes(GraphicsContext gc) {
        ...
        gc.fillArc(10, 110, 30, 30, 45, 50, ArcType.OPEN);
        gc.fillArc(60, 110, 30, 30, 45, 50, ArcType.CHORD);
        gc.fillArc(110, 110, 30, 30, 45, 50, ArcType.ROUND);
